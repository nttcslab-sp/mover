...
template<typename T>
std::pair<bool, bool> detect_overlap_and_correct_(
    const std::vector<unsigned int>& ref_symbol,
    const std::vector<std::pair<T, T>>& ref_interval,
    const unsigned int& hyp_symbol,
    const std::pair<T, T>& hyp_interval
){
    auto ref_symbol_it = ref_symbol.begin();
    auto ref_interval_it = ref_interval.begin();
    bool overlapped = false;
    bool correct = false;
    while (ref_symbol_it != ref_symbol.end() && ref_interval_it != ref_interval.end()) {
        if (overlaps(*ref_interval_it, hyp_interval)) {
            overlapped = true;
            if (hyp_symbol == *ref_symbol_it){
                correct = true;
                break;
            }
        }
    ...
    return std::make_pair(overlapped, correct);
}

template<typename T>
LevenshteinStatistics time_constrained_levenshtein_distance_with_alignment_for_rover_(
        const std::vector<std::vector<unsigned int>> reference,
        const std::vector<unsigned int> hypothesis,
        const std::vector <std::vector<std::pair<T, T>>> reference_timing,
    ...
    // This is required when the reference intervals don't have increasing end times
    // T ref_end_time = reference_timing[0].second;
    T ref_end_time = std::max_element(
            reference_timing[0].begin(), reference_timing[0].end(),
    ...
    for (unsigned int ref_index = 0; ref_index < reference.size(); ref_index++) {
        ...
        auto ref_interval = reference_timing[ref_index];

        assert(ref_symbol.size() == ref_interval.size());
        // ref_end_time = std::max(ref_interval.second, ref_end_time);
        ref_end_time = std::max(
            std::max_element(ref_interval.begin(), ref_interval.end(), [](const std::pair<T, T>& a, const std::pair<T, T>& b) {
        ...
        if (prune) for (; start_hyp_index < hypothesis.size() + 1; start_hyp_index++) {
            // if (hypothesis_timing[start_hyp_index - 1].second > ref_interval.first) break;
            if (std::any_of(
                ref_interval.begin(), ref_interval.end(),
                [&](const std::pair<T, T>& a){ return hypothesis_timing[start_hyp_index - 1].second > a.first; }
            )) break;
        }
        ...
        for (; hyp_index < hypothesis.size() + 1; hyp_index++) {
            ...
            auto ins_or_del = std::min(
                    ...
                    row[hyp_index - 1] + (hyp_symbol == ALIGNMENT_EPS ? cost_cor : cost_ins), // left -> insertion
                    prev_row[hyp_index] + (has_element_(ref_symbol, ALIGNMENT_EPS) ? cost_cor : cost_del)// up -> deletion
            );
            // if (overlaps(ref_interval, hyp_interval)) {
            //     row[hyp_index] = std::min(
            //             ins_or_del,
            ...
            //             prev_row[hyp_index - 1] + (has_element_(ref_symbol, hyp_symbol) ? cost_cor : cost_sub)
            //     );
            // } else {
            //     row[hyp_index] = ins_or_del;
            // }
            //
            auto overlapped_and_correct = detect_overlap_and_correct_(ref_symbol, ref_interval, hyp_symbol, hyp_interval);
            if (overlapped_and_correct.first){
                row[hyp_index] = std::min(
                        ins_or_del,
                        // diagonal -> correct or substitution
                        prev_row[hyp_index - 1] + (overlapped_and_correct.second? cost_cor : cost_sub)
                );
            } else {
                row[hyp_index] = ins_or_del;
            }
        ...
        if (prune) {
            ...
            if (ref_index < reference_timing.size() - 1) {
                for (; hyp_index < hypothesis.size() + 1; hyp_index++) {
                    ...
                    // if (hypothesis_timing[hyp_index - 1].first > reference_timing[ref_index + 1].second) break;
                    if (std::all_of(
                        reference_timing[ref_index + 1].begin(), reference_timing[ref_index + 1].end(),
                        [&](const std::pair<T, T>& a){ return hypothesis_timing[hyp_index - 1].first > a.second; }
                    )) break;
                }
            } else {
                // We reached the end. There are no following rows which we can respect while forwarding. There can only be
    ...
    while (ref_index > 0 || hyp_index > 0) {
        if (ref_index == 0) {
            ...
            unsigned int cost_insertion = matrix[ref_index][hyp_index - 1] + (hypothesis.at(hyp_index - 1) == ALIGNMENT_EPS? cost_cor: cost_ins);
            unsigned int cost_deletion = matrix[ref_index - 1][hyp_index] + (has_element_(reference.at(ref_index - 1), ALIGNMENT_EPS)? cost_cor: cost_del);

            // if (overlaps(hypothesis_timing[hyp_index - 1], ) {
            //     unsigned int cost_cor_sub = matrix[ref_index - 1][hyp_index - 1];
            //     if (has_element_(reference.at(ref_index - 1), hypothesis.at(hyp_index - 1))) cost_cor_sub += cost_cor;
            ...
            //         if (! has_element_(reference.at(ref_index), hypothesis.at(hyp_index))) statistics.substitutions++;
            //         else statistics.correct++;
            //         continue;
            //     }
            auto overlapped_and_correct = detect_overlap_and_correct_(reference.at(ref_index - 1), reference_timing[ref_index - 1], hypothesis.at(hyp_index - 1), hypothesis_timing[hyp_index - 1]);
            if (overlapped_and_correct.first) {
                unsigned int cost_cor_sub = matrix[ref_index - 1][hyp_index - 1] + (overlapped_and_correct.second? cost_cor : cost_sub);

                if (cost_cor_sub < std::min(cost_deletion, cost_insertion)) {
                    statistics.alignment.push_back(std::make_pair(--ref_index, --hyp_index));
                    // Modified by Kamo
                    // if (reference.at(ref_index) != hypothesis.at(hyp_index)) statistics.substitutions++;
                    if (! overlapped_and_correct.second) statistics.substitutions++;
                    else statistics.correct++;
                    continue;
                }
            }

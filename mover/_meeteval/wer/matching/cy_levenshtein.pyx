...
cdef extern from "levenshtein.h":
    ...
    LevenshteinStatistics time_constrained_levenshtein_distance_with_alignment_for_rover_[T](
            vector[vector[uint]] reference,
            vector[uint] hypothesis,
            vector[vector[pair[T, T]]] reference_timing,
            vector[pair[T, T]] hypothesis_timing,
            uint cost_del,
            uint cost_ins,
            uint cost_sub,
...
def obj2vec(*args, alignment_eps="*"):
    ...
    sym2int = {v: i for i, v in enumerate({c for a in args for b in a for c in (b if isinstance(b, list) else [b])})}
    sym2int[alignment_eps] = ALIGNMENT_EPS
    return [[[sym2int[c] for c in b] if isinstance(b, list) else sym2int[b] for b in a] for a in args]


def list_to_list_list(vec):
    return [v if isinstance(v, list) else [v] for v in vec]
...
cdef bool _timing_is_integer2(timing):
    return all(isinstance(t2[0], int) and isinstance(t2[1], int) for t in timing for t2 in t)
...
cdef bool _timings_sorted2(vector[vector[pair[timing_type, timing_type]]] timing):
    cdef timing_type last_begin = 0
    cdef vector[pair[timing_type, timing_type]] t
    for t in timing:
        # Assume having only an element
        assert len(t) == 1
        if t[0].first < last_begin:
            return False
        last_begin = t[0].first
    return True
...
cdef _time_constrained_levenshtein_distance_with_alignment_for_rover(
        reference,  # list[int] or list[list[int]]
        hypothesis,  # list[int]
        reference_timing: vector[vector[pair[timing_type, timing_type]]],  # list[tuple[int, int]]
        hypothesis_timing: vector[pair[timing_type, timing_type]],  # list[tuple[int, int]]
        cost_del = 1,
        cost_ins = 1,
        cost_sub = 1,
    ...
    if prune == 'auto':
        assert all(len(_reference_timing) == 1 for _reference_timing in reference_timing), "assert1"
        prune = _timings_sorted2(reference_timing) and _timings_sorted(hypothesis_timing)
        logging.debug(f'prune is set to "auto". Using prune={prune!r}.')
    elif prune:
        assert all(len(_reference_timing) == 1 for _reference_timing in reference_timing), "assert2"
    ...
    reference, hypothesis = obj2vec(reference, hypothesis, alignment_eps=alignment_eps)

    cdef dict statistics

    statistics = time_constrained_levenshtein_distance_with_alignment_for_rover_(
        reference,
        hypothesis,
        reference_timing,
        hypothesis_timing,
...
def time_constrained_levenshtein_distance_with_alignment_for_rover(
        reference,  # list[int]
        hypothesis,  # list[int]
        reference_timing,  # list[tuple[int, int]]
        hypothesis_timing,  # list[tuple[int, int]]
    ...
    if len(reference_timing) == 0:
        reference_timing = [[]]
    elif not isinstance(reference_timing[0], list):
        reference_timing = [[x] for x in reference_timing]

    assert all(len(x) == len(y) for x, y in zip(reference, reference_timing))

    if _timing_is_integer2(reference_timing) and _timing_is_integer(hypothesis_timing):
        return _time_constrained_levenshtein_distance_with_alignment_for_rover[int](
            reference, hypothesis, reference_timing, hypothesis_timing,
            cost_del, cost_ins, cost_sub, cost_cor, prune, alignment_eps
        )
    else:
        return _time_constrained_levenshtein_distance_with_alignment_for_rover[double](
            reference, hypothesis, reference_timing, hypothesis_timing,
            cost_del, cost_ins, cost_sub, cost_cor, prune, alignment_eps
        )

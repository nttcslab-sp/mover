...
def split_words(
    ...
    def split_entry(x):
        ...
        #     return x
            ...
            assert all(len(y.split()) == 1 for y in x)
            return [x]
        else:
            raise TypeError(x)
...
def check_timestamps_valid(seglst: 'SegLST', name=None):
    for s in seglst:
        if not (isinstance(s['end_time'], list) and isinstance(s["start_time"], list)) and s['end_time'] < s['start_time']:
            raise ValueError(
...
def _preprocess_single(
    ...
    if return_self_overlap and 'start_time' in segments.T.keys() and 'end_time' in segments.T.keys():
...
def preprocess(
    ...
    reference, reference_self_overlap = _preprocess_single(
        ...
        segment_representation=segment_representation,
        return_self_overlap=return_self_overlap,
    )

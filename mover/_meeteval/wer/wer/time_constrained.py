...
from mover._meeteval.wer.preprocess import split_words, words_to_int, add_segment_index, preprocess
from mover._meeteval.wer.matching.cy_levenshtein import time_constrained_levenshtein_distance_with_alignment_for_rover
...
def character_based_points(interval, words):
    ...
    intervals = [
        ([(_interval[1] + _interval[0]) / 2 for _interval in zip(interval[0], interval[1])],) * 2
        if isinstance(interval[0], list) else
...
def align(
    ...
    if len(reference) == 0 and len(hypothesis) == 0:
        ...
        alignment = [(None, value) for value in hypothesis]
    elif len(hypothesis) == 0:
        alignment = [(value, None) for value in reference]
    else:
        ...
        # list[tuple[int, int]] or list[tuple[list[int], list[int]]]
        hypothesis_timing = hypothesis_.T['start_time', 'end_time']

        # to list[list[str]]
        reference_words = [w if isinstance(w, list) else [w] for w in reference_words]
        # to list[list[tuple[int, int]]]
        reference_timing = [[(a, b) for a, b in zip(t[0], t[1])] if isinstance(t[0], list) else [t] for t in reference_timing]
        assert all(len(w) == len(t) for w, t in zip(reference_words, reference_timing))

        alignment = time_constrained_levenshtein_distance_with_alignment_for_rover(
            reference_words, hypothesis_words, reference_timing, hypothesis_timing, prune=prune, alignment_eps=alignment_eps
        )['alignment']
